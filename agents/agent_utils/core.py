...
"""
Code modified from https://github.com/RobvanGastel/rnn-sac/tree/main
"""
...
from architectures.cnn import CNN, CNNLayer
...
class Memory(nn.Module):
    def __init__(self, obs_dim, act_dim, device,
        ...
        conv1 = CNNLayer(obs_dim, 32, 8, 2)
        conv2 = CNNLayer(32, 64, 8, 2)
        conv3 = CNNLayer(64, 64, 3)
        conv_feature = Linear(576, hidden_size)
        self.net = CNN([conv1, conv2, conv3], conv_feature)
        # +1 for the reward
    ...
    def _one_hot(self, act):
        return torch.eye(self.act_dim)[act.long().cpu(), :].to(self.device)

    def forward(self, obs, prev_act, prev_rew, hid_in,
                training=False):

        act_enc = self._one_hot(prev_act)
        obs_enc = self.activation(self.net(obs)[0])
...
class QNetwork(nn.Module):
    ...
    def forward(self, memory_emb):
        mem = self.activation(self.net(memory_emb)[0])
        q = self.q(mem)
        return q
...
class ActorCritic(nn.Module):
    def __init__(self, observation_space, action_space, device,
        ...
        obs_dim = observation_space['image'].shape[-1]
